...
private:
	...
	struct map_t {
		char at(const pos_t& p) const { return grid[p.c * height + p.r]; }
		char& at(const pos_t& p) { return grid[p.c * height + p.r]; }
...
private:
	...
	void setMap(const vector<string>& lines) {
		m_map.height = (int)lines.size();
		m_map.width = (int)lines.front().size();
		for (auto& line : lines) {
	...
	loop_t findLoop() {
		...
		if (m_map.at(pos + down) == '|' || m_map.at(pos + down) == 'L' || m_map.at(pos + down) == 'J') {
			dir = down;
		}
		else if (m_map.at(pos + up) == '|' || m_map.at(pos + up) == 'F' || m_map.at(pos + up) == '7') {
			dir = up;
		}
		else if (m_map.at(pos + left) == '-' || m_map.at(pos + left) == 'F' || m_map.at(pos + left) == 'L') {
			dir = left;
		}
		else if (m_map.at(pos + right) == '-' || m_map.at(pos + right) == '7' || m_map.at(pos + right) == 'J') {
			dir = right;
		}
